...
# FOLDERS
setwd("/Users/jonathanlatner/OneDrive/SECCOPA/projects/wages_contyp/support_files/simulation/")
...
df_first_event <- suppressWarnings(df_first_event %>%
                                           ...
                                           mutate(event_t_p = ifelse(perm == 1 & lag(temp,1) == 1 & row_number()>1 & year == lag(year,1,NA)+year_lag, yes = 1, no = 0), # identify treatment
                                                  ...
                                                  event_t_p_time = ifelse(event_t_p_time < -2, yes = -3,
...
df_first_event <- suppressWarnings(df_first_event %>%
                                           ...
                                           mutate(event_p_t = ifelse(temp == 1 & lag(perm,1) == 1 & row_number()>1 & year == lag(year,1,NA)+year_lag, yes = 1, no = 0),
                                                  ...
                                                  event_p_t_time = ifelse(event_p_t_time < -2, yes = -3,
...
# df_events_all %>% select(pid,year,transseq,temp,perm,wage,eventyear,eventtime,event_p_t_yes,event_p_t_yes,event_t_p_yes,event_p_t_time_pos)
...
t <- xtable(df_events_all %>% select(pid,year,transseq,temp,perm,wage,eventyear,eventtime,event_p_t_yes,event_p_t_yes,event_t_p_yes,event_p_t_time_pos,event_t_p_time_pos), digits = c(0))
print(t,
