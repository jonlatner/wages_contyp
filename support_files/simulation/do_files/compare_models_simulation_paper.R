...
df_example_2 <- df_example %>%
        ...
        mutate(ln_hourly_wage = ifelse(pid==1, yes = ln_hourly_wage + (year-1)*30 + perm*30, no = ln_hourly_wage),
...
df_example_3 <- df_example %>%
        ...
        mutate(ln_hourly_wage = ifelse(pid==1, yes = ln_hourly_wage + (year-1)*30 + perm*-20, no = ln_hourly_wage),
               ln_hourly_wage = ifelse(pid==2, yes = ln_hourly_wage + (year-1)*20 + perm*30, no = ln_hourly_wage),
        )

df_original <- rbind(df_example_1,df_example_2,df_example_3)
# rbind(df_example_1,df_example_2,df_example_3)
...
counterfactual_feis <- df_graph %>%
        group_by(example,ID,temp) %>%
        mutate(ln_hourly_wage = ifelse(example == "Simulation 1" & temp==1, yes = mean(ln_hourly_wage), no = NA)) %>%
        group_by(example,ID) %>%
        mutate(ln_hourly_wage = ifelse(example == "Simulation 1" & first(!is.na(ln_hourly_wage)), yes = first(ln_hourly_wage), no = last(ln_hourly_wage))) %>%
        ungroup() %>%
        mutate(ln_hourly_wage = ifelse(example > "Simulation 1" & pid==1, yes = 100 + (year-1)*30, no = ln_hourly_wage),
               ln_hourly_wage = ifelse(example > "Simulation 1" & pid==2, yes = 270 + (year-1)*20, no = ln_hourly_wage),
               counterfactual = "FEIS",
               )
...
counterfactual_fe_if <- df_graph %>%
        ...
        ungroup() %>%
        mutate(ln_hourly_wage = ifelse(example > "Simulation 1" & pid==1, yes = 100 + (year-1)*30, no = ln_hourly_wage),
               ln_hourly_wage = ifelse(example > "Simulation 1" & pid==1 & year > 3, yes = 160, no = ln_hourly_wage),
               ln_hourly_wage = ifelse(example > "Simulation 1" & pid==2, yes = 300 + (year-1)*20, no = ln_hourly_wage),
               ln_hourly_wage = ifelse(example > "Simulation 1" & pid==2 & year > 3, yes = 340, no = ln_hourly_wage),
               counterfactual = "FE + IF",
        )

counterfactual_fe_if
...
texreg(
        list(model_fe_1, model_feis_1, model_fe_if_1, model_fe_2, model_feis_2, model_fe_if_2, model_fe_3, model_feis_3, model_fe_if_3),
        custom.header = list("Simulation 1" = 1:3, "Simulation 2" = 4:6, "Simulation 3" = 7:9),
        custom.model.names = c("FE", "FEIS", "FE+IF", "FE", "FEIS", "FE+IF", "FE", "FEIS", "FE+IF"),
        table = FALSE,
        custom.coef.map = list(
                "temp"="Temp",
