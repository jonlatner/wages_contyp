...
df_uk <- readRDS(paste0(data_files, "UK/uk_sample_all_temporary.rds")) # all non permanent jobs are temporary (including seasonal)
...
# Step 1 - Panel years between 2000 and 2018 ----
...
# Step 3 - keep if labor force participant ----
...
# Step 4 - unemployed or employed with contract ----
...
# Step 5 - unemployed or employed with wages ----
df_sample_5 <- df_sample_4 %>%
  filter(unmp == 1 | (unmp == 0 & !is.na(wages)))
...
# Step 6 - Unemployed or employed and monthly hours between 40 and 320 ----
#  (i.e. no less than 10 hours per week and no more than 80 hours per week)

df_sample_6 <- df_sample_5 %>%
  filter(unmp == 1 | (unmp == 0 & (hours >= 40 & hours <= 320)))

df_sample_5 %>% 
  group_by(unmp,country) %>% 
  summarise(min = min(hours, na.rm = TRUE), 
            max = max(hours, na.rm = TRUE))

df_sample_5 %>% 
  group_by(unmp, country) %>%
  summarise(min = quantile(hours, probs = 0.01, na.rm = TRUE), 
            max = quantile(hours, probs = 0.99, na.rm = TRUE))

df_sample_5 %>% group_by(unmp,country) %>% 
  summarise(hours = min(hours), 
            wages = min(wages))
...
# Step 6 - keep if non missing values on key demographic variables ----
...
# Step 7 - Unemployed or employed with Hourly wages within the top/bottom 0.005 percentile ----
...
df_sample_8 <- merge(df_sample_7,df_unmp_rate, by = c("country","year")) %>%
        arrange(country,pid,year)
...
df_sample_9 <- merge(df_sample_8,df_exchange, by = c("country"))

df_sample_9 %>% 
  filter(unmp == 0) %>% 
  summarise(wage = min(hourly_wage),
            hours = min(hours))

# convert wages to U.S. dollars 
df_sample_9 <- df_sample_9 %>%
        # filter(unmp == 1 | (unmp==0 & hourly_wage > 0 & hours > 0)) %>% # employed and wages are greater than 0
        mutate(wages_dollars = wages/exchange,
               hourly_wage_dollars = hourly_wage/exchange)

df_wages_1 <- df_sample_9 %>%
        filter(unmp==0 & hourly_wage > 0 & hours > 0) %>% # employed and wages are greater than 0
...
df_sample_10 <- merge(df_sample_9,df_wages_1,by=c("country","year")) %>%
        arrange(country,pid,year)

# keep if unemployed or monthly wages not missing
df_sample_11 <- df_sample_10 %>%
...
saveRDS(df_sample_11, file = paste0(data_files, "03a_df_sample.rds"))

df_filter_steps <- rbind(step_0,step_1,step_2,step_3,step_4,step_5,step_6,step_7,step_8)
saveRDS(df_filter_steps, file = paste0(data_files, "3a_df_filter_steps.rds"))
