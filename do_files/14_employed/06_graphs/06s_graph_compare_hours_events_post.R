...
# Sensitivity sample (remain employed)
df_multiple_unmp <- readRDS(paste0(results,"employed/df_yhat_multiple_events_unmp.rds"))
df_multiple_unmp <- df_multiple_unmp %>%
  filter(country!="NE-LISS") %>%
  mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country))
df_multiple_unmp$type = "Sensitivity sample (remain employed after transition)"

df_employed_1 <- df_multiple_unmp
...
df_multiple_unmp <- df_multiple_unmp %>%
  filter(country!="NE-LISS") %>%
  mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country))
df_multiple_unmp$type = "Main sample"

df_employed_2 <- df_multiple_unmp

df_append <- rbind(df_employed_1,df_employed_2)
...
ggplot(data = df_graph, aes(x = post, y = estimate, color = type, group = type)) +
        facet_nested_wrap(~ event + country_name, scales = "free", ncol = 8) +
        geom_line(size = 1) +
...
ggsave(paste0(graphs,"graph_sensitivity_employed_post.pdf"), height = 6, width = 9, plot = last_plot())
