...
df_contyp_2 <- readRDS(paste0(results,"df_yhat_multiple_events_contyp_UK.rds"))
...
df_unmp_2 <- readRDS(paste0(results,"df_yhat_multiple_events_unmp_UK.rds"))
df_unmp_2 <- df_unmp_2 %>%
        filter(country=="UK") %>%
        rename(model = event) %>%
        mutate(type = "FTC only")
...
ggsave(paste0(graphs,"graph_sensitivity_UK_paper.pdf"), height = 6, width = 9, plot = last_plot())
