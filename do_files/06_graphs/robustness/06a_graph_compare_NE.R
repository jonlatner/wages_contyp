...
df_contyp_2 <- readRDS(paste0(results,"df_yhat_multiple_events_contyp.rds"))
...
df_unmp_2 <- readRDS(paste0(results,"df_yhat_multiple_events_unmp.rds"))
df_unmp_2 <- df_unmp_2 %>%
        filter(country=="NE-LISS") %>%
        mutate(type = "LISS")
...
ggsave(paste0(graphs,"graph_sensitivity_NE.pdf"), height = 4, width = 8, plot = last_plot())
ggsave(paste0(graphs,"graph_sensitivity_NE_paper.pdf"), height = 6, width = 9, plot = last_plot())
