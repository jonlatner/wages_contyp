...
# Contyp
df_contyp_f <- readRDS(paste0(results,"df_yhat_multiple_events_contyp_gender_F.rds"))
df_contyp_f <- df_contyp_f %>%
        filter(country!="NE-LISS") %>%
        mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country),
               gender = "Female")

df_contyp_m <- readRDS(paste0(results,"df_yhat_multiple_events_contyp_gender_M.rds"))
...
# Unemployment
df_unmp_f <- readRDS(paste0(results,"df_yhat_multiple_events_unmp_gender_F.rds"))
df_unmp_f <- df_unmp_f %>%
        filter(country!="NE-LISS") %>%
        mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country),
               gender = "Female")

df_unmp_m <- readRDS(paste0(results,"df_yhat_multiple_events_unmp_gender_M.rds"))
df_unmp_m <- df_unmp_m %>%
        filter(country!="NE-LISS") %>%
        mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country),
               gender = "Male")
...
# Append
df_append <- rbind(df_contyp_f,df_contyp_m,df_unmp_f,df_unmp_m)
...
ggsave(paste0(graphs,"graph_post_event_gender.pdf"), height = 6, width = 9, plot = last_plot())
...
ggplot(data = df_graph, aes(x = post, y = estimate, color = event, group = event)) +
        facet_grid(gender ~ country_name, scales = "free_y") +
        geom_line(size = 1) +
        # geom_hline(yintercept = 0) +
        # geom_vline(xintercept = 0) +
        # scale_y_continuous(limits = c(-.35,.35), breaks = seq(-.3,.3,.2)) +
...
ggsave(paste0(graphs,"graph_contyp_gender_event_contyp.pdf"), height = 5, width = 8, plot = last_plot())
...
ggsave(paste0(graphs,"graph_contyp_gender_event_unmp.pdf"), height = 5, width = 8, plot = last_plot())
...
ggsave(paste0(graphs,"graph_contyp_gender_event_t_p.pdf"), height = 4, width = 8, plot = last_plot())
...
ggsave(paste0(graphs,"graph_contyp_gender_event_p_t.pdf"), height = 4, width = 8, plot = last_plot())
...
ggsave(paste0(graphs,"graph_contyp_gender_event_u_t.pdf"), height = 4, width = 8, plot = last_plot())
...
ggsave(paste0(graphs,"graph_contyp_gender_event_u_p.pdf"), height = 4, width = 8, plot = last_plot())
