...
df_sample_1 <- df_sample_0 %>%
        ...
        select(country,pid,year,ln_hourly_wage,unmp,temp,matches("event_4"))
...
for(c in country) {
        ...
        df_dummy <- dummy(x = df_country$post_event_4,sep = "_")
...
iv_vars = c("pre_event_4",
            "event_4", 
            "post_event_4_1", "post_event_4_2", "post_event_4_3"
)
country <- c("IT","NE-LSP")
for(c in country) {
        print(c)
        ...
        # create year variables
        df_country <- df_country %>%
                mutate(post_event_4 = ifelse(post_event_4 > 3, yes = 3, no = post_event_4))
...
write.csv(df_model_yhat,file = paste0(results,"yhat_event_4.csv"))
