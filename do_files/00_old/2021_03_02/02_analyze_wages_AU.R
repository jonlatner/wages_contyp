...
df_au <- readRDS(paste0(data_files, "df_sample_clean.rds")) 
df_au <- df_au %>%
        filter(country == "AU")
summary(df_au)
...
df_au$edu_cat <- relevel(df_au$edu_cat,ref = 2)
df_au$age_cat <- relevel(df_au$age_cat,ref = 2)
...
df_dummy <- dummy(x = df_au_cntr$year)
df_au_cntr <- cbind(df_au_cntr, df_dummy)

df_dummy <- dummy(x = df_au_cntr$age_cat)
df_au_cntr <- cbind(df_au_cntr, df_dummy)

df_dummy <- dummy(x = df_au_cntr$edu_cat)
df_au_cntr <- cbind(df_au_cntr, df_dummy)
rm(df_dummy)
...
for(i in iv){
        for(f in temp){
                df_au_cntr$test <- df_au_cntr[[i]]*df_au_cntr[[f]]
                df_au_cntr$test <- as.numeric(df_au_cntr$test)
                names(df_au_cntr)[names(df_au_cntr) == 'test'] <- paste(i,f,sep = "_")
        }
}
rm(i,f,iv,temp)
...
for (v in vars) {
        df_au_cntr$test <- group.center(df_au_cntr[[v]], df_au_cntr$pid) # create new variable (group centered)
        df_au_cntr$test <- as.numeric(df_au_cntr$test) # make it numeric (probably not necessary)
        df_au_cntr[[v]] <- NULL # drop old variable
        names(df_au_cntr)[names(df_au_cntr) == "test"] <- paste0(v) # rename new variable with old variable
        
}

# View(filter(df_au_cntr,age_cat1==1&post_temp==1) %>% select(pid,year,temp,post_temp,matches("age_cat1")))
# View(filter(df_au_cntr,pid==1242) %>% select(pid,year,temp,post_temp,matches("age_cat1")))

# independent variables -----

iv_plm <-   "unmp + temp + post_temp + 
temp*as.factor(age_cat) + post_temp*as.factor(age_cat) +
temp*as.factor(edu_cat) + post_temp*as.factor(edu_cat) +
temp*as.factor(female) + post_temp*as.factor(female) +
factor(year)"
...
edu_cat3_temp + edu_cat3_post_temp +
female_temp + female_post_temp +
year2002 + year2003 + year2004 + year2005 + year2006 + year2007 + year2008 + year2009 + year2010 + year2011 + year2012 + year2013 + year2014 + year2015 + year2016 + year2017 + year2018"
...
# stargazer(plm_model,ols_model,type = "text", omit = c("year"))
...
write.csv(df_yhat, file = paste0(results, "wages_mfx_au.csv"))
write.csv(df_model, file = paste0(results, "wages_results_au.csv"))
...
# ggsave(filename = paste0(graphs,"graph_ch_wages.pdf"), plot = last_plot(), height = 8, width = 10, units = "in")
