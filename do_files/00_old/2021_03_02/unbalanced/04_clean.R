...
filter(df_sample_1,pid==8603) %>% select(country,pid,year,emp_status,matches("temp"))
...
filter(df_sample_2,pid==8603) %>% select(country,pid,year,emp_status,matches("temp"))

# filter(df_sample_2,pid==18804)
# filter(df_sample_2,pid==8201202)

df_sample_4 <- df_sample_2 %>%
        arrange(country, study_period, pid, year) %>%
        group_by(country, study_period, pid) %>%
