...
df_ch <- readRDS(paste0(data_files, "df_sample_clean.rds")) 
df_ch <- df_ch %>%
        filter(country == "CH")
summary(df_ch)
...
# View(filter(df_ch_cntr,age_cat1==1&post_temp==1) %>% select(pid,year,temp,post_temp,matches("age_cat1")))
# View(filter(df_ch_cntr,pid==1242) %>% select(pid,year,temp,post_temp,matches("age_cat1")))
...
write.csv(df_yhat, file = paste0(results, "prestige_mfx_ch.csv"))
write.csv(df_model, file = paste0(results, "prestige_results_ch.csv"))
