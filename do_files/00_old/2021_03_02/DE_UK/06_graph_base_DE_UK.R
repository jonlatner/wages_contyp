...
graphs = "projects/mobility/graphs/DE_UK/"
...
df_model_output <- readRDS(paste0(results, "results_a_predicted_DE_UK.rds"))

# df_model_output_gender <- readRDS(paste0(results, "results_a_predicted_DE_UK_gender.rds"))
# df_model_output <- bind_rows(df_model_output,df_model_output_gender)

# graph data ----

df_graph <- df_model_output %>%
        filter(period==0) %>%
        filter((country == "DE" & study_period > 1995) | country == "UK") %>%
...
ggplot(data = df_graph, aes(x = study_period, y = fit)) +
        ...
        geom_line(size = 1) +
        geom_point(size = 2) +
        scale_x_continuous(breaks =seq(1990,2012,4)) +
...
ggsave(paste0(graphs,"graph_base_model_period_0.pdf"), height = 5, width = 7, plot = last_plot())
