...
step1 <- with(df_sample_1,table(country,year))
...
for(c in country) {
        ...
        for(y in year) {
                print(y)
                df_period <- filter(df_country, year >= y & year <= y + 4)
...
# we also exlclude study period 2005 - 2009 because 2009 is missing, leaving only 4 year study period
country <- c("UK")
for(c in country) {
        df_country <- df_sample_1
        df_country <- df_country %>%
        ...
        for(y in year) {
                ...
                df_period <- group_by(df_period, pid) %>%
                        ...
                        mutate(sample_unmp = ifelse(row_number()==1 & unmp == 1, yes = 1, no = 0),
                               sample_unmp = first(sample_unmp)) %>%
                        ungroup() %>%
                        filter(total==4 & first == min(year) & last == max(year)) %>%
                        mutate(study_period = min(year)) %>%
                        select(-first,-last)
...
step2a <- df_sample_2 %>%
        ...
        filter(row_number()==n()) %>%
        ungroup()
step2c <- with(step2a,table(country))

# duplicates
step3a <- df_sample_2 %>%
        group_by(pid,study_period) %>%
        filter(row_number()==n()) %>%
        ungroup()

step3b <- with(step3a,table(country))

df_filter_steps <- rbind(step0b,step1b,step2b,step2c,step3b)
df_filter_steps
