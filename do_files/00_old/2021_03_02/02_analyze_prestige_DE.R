...
df_de <- readRDS(paste0(data_files, "df_sample_clean.rds")) 
df_de <- df_de %>%
        filter(country == "DE")
summary(df_de)
...
df_de$edu_cat <- relevel(df_de$edu_cat,ref = 2)
df_de$age_cat <- relevel(df_de$age_cat,ref = 2)

iv_plm <-   "unmp + temp + post_temp + factor(year)"
iv_plm <-   "unmp + 
...
df_dummy <- dummy(x = df_de_cntr$year)
df_de_cntr <- cbind(df_de_cntr, df_dummy)

df_dummy <- dummy(x = df_de_cntr$age_cat)
df_de_cntr <- cbind(df_de_cntr, df_dummy)

df_dummy <- dummy(x = df_de_cntr$edu_cat)
df_de_cntr <- cbind(df_de_cntr, df_dummy)
rm(df_dummy)
...
for(i in iv){
        for(f in temp){
                df_de_cntr$test <- df_de_cntr[[i]]*df_de_cntr[[f]]
                df_de_cntr$test <- as.numeric(df_de_cntr$test)
                names(df_de_cntr)[names(df_de_cntr) == 'test'] <- paste(i,f,sep = "_")
        }
}
rm(i,f,iv,temp)
...
for (v in vars) {
        df_de_cntr$test <- group.center(df_de_cntr[[v]], df_de_cntr$pid) # create new variable (group centered)
        df_de_cntr$test <- as.numeric(df_de_cntr$test) # make it numeric (probably not necessary)
        df_de_cntr[[v]] <- NULL # drop old variable
        names(df_de_cntr)[names(df_de_cntr) == "test"] <- paste0(v) # rename new variable with old variable
        
}

# View(filter(df_de_cntr,age_cat1==1&post_temp==1) %>% select(pid,year,temp,post_temp,matches("age_cat1")))
# View(filter(df_de_cntr,pid==1242) %>% select(pid,year,temp,post_temp,matches("age_cat1")))
...
write.csv(df_yhat, file = paste0(results, "prestige_mfx_de.csv"))
write.csv(df_model, file = paste0(results, "prestige_results_de.csv"))
...
# ggsave(filename = paste0(graphs,"graph_de_wages.pdf"), plot = last_plot(), height = 8, width = 10, units = "in")
