...
df_uk <- df_uk %>%
        mutate(prestige = ifelse(is.na(prestige), yes = 0, no = prestige))

iv_plm <-   "unmp + tmp + ftc + post_ftc + factor(year)"
iv_plm <-   "unmp + tmp +
...
saveRDS(df_yhat, file = paste0(results, "uk_prestige_post_ftc.rds"))
