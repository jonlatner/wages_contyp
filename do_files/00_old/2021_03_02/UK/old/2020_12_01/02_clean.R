...
df_sample_4 <- df_sample_3 %>%
        ...
        mutate(end_ftc = na.locf(end_ftc,na.rm = FALSE),
               ...
               post_ftc = ifelse(is.na(post_ftc), yes = 0, no = post_ftc)
        ) %>%
        mutate(end_ftc = na.locf(end_ftc,na.rm = FALSE)) %>%
        ungroup() %>%
        arrange(pid,year)
...
df_sample_5 <- df_sample_4 %>%
        arrange(pid, year) %>%
        group_by(pid) %>%
        mutate(end_ftc_first = end_ftc) %>%
        arrange(pid,end_ftc) %>%
        mutate(end_ftc_first = first(end_ftc)) %>%
        arrange(pid,year) %>%
        mutate(post_ftc_first = cumsum(post_ftc),
               post_ftc_first = ifelse(post_ftc_first>1, yes = 1, no = post_ftc_first),
               post_ftc_first = ifelse(post_ftc_first==1, yes = year-end_ftc_first, no = NA),
               post_ftc_first = ifelse(is.na(post_ftc_first), yes = 0, no = post_ftc_first)) %>%
        ungroup()

# with(subset(df_sample_5, post_ftc>0), prop.table(table(post_ftc)))
...
saveRDS(df_sample_7, file = paste0(data_files, "uk_bhps.rds"))
