...
results = "results/sensitivity/AU_FTC/"
...
df_event_t_p <- df_original %>%
        select(country,pid,pidseq,year,age,ln_hourly_wage,hourly_wage,unmp,temp,perm,matches("event_t_p"),unemployment_rate) %>%
        rename(event_time_pos = event_t_p_time_pos) %>%
        filter(!is.na(event_time_pos))

df_event_u_p <- df_original %>%
        select(country,pid,pidseq,year,age,ln_hourly_wage,hourly_wage,unmp,temp,perm,matches("event_u_p"),unemployment_rate) %>%
        rename(event_time_pos = event_u_p_time_pos) %>%
        filter(!is.na(event_time_pos))

df_event_u_t <- df_original %>%
        select(country,pid,pidseq,year,age,ln_hourly_wage,hourly_wage,unmp,temp,perm,matches("event_u_t"),unemployment_rate) %>%
        rename(event_time_pos = event_u_t_time_pos) %>%
        filter(!is.na(event_time_pos))

df_event_p_t <- df_original %>%
        select(country,pid,pidseq,year,age,ln_hourly_wage,hourly_wage,unmp,temp,perm,matches("event_p_t"),unemployment_rate) %>%
...
for(c in country_ann) {
        ...
        for (e in event) {
                ...
                model <- feis(hourly_wage ~ age + unemployment_rate + event_time_pos + as.factor(year) | 1,
                              data = df_country, 
                              robust = TRUE,
                              id = "pidseq")
                
                # assign(paste0("model_hourly_wages_raw_event_",e,"_country_",c),model)
                save(model, file=paste0(results,"model_hourly_wages_raw_event_",e,"_country_",c,".Rdata"))
...
saveRDS(df_yhat, file = paste0(results, "df_yhat_hourly_wages_raw.rds"))
