...
iv_vars = c("event_time_pos_1 + event_time_pos_2 + event_time_pos_3 + event_time_pos_4 + event_time_pos_5 + event_time_pos_6 + event_time_pos_7 + event_time_pos_8")
...
screenreg(list(model_event_1_country_DE,model_event_2_country_DE,model_event_3_country_DE,model_event_4_country_DE),
          omit.coef = c("year"), 
          custom.coef.names = c("Pre event (-2)", "Pre event (-1)", "Event", "Post event (+1)", "Post event (+2)", "Post event (+4)", "Post event (+4)", "Post event (+5)")
)

texreg(list(model_event_1_country_DE,model_event_4_country_DE,model_event_2_country_DE,model_event_3_country_DE),
       custom.model.names = c("Event 1 (T-P)", "Event 4 (P-T)", "Event 2 (U-P)", "Event 3 (U-T)"),
       custom.coef.names = c("Pre event (-2)", "Pre event (-1)", "Event", "Post event (+1)", "Post event (+2)", "Post event (+4)", "Post event (+4)", "Post event (+5)"),
       file = paste0(tables,"table_model_output_country.tex"),
       table = FALSE,
...
for(c in country) {
        for (e in event) {
        model_event <- get(paste0("model_event_",e))
...
ggsave(paste0(graphs,"graph_model_output_country.pdf"), height = 4, width = 8, plot = last_plot())
