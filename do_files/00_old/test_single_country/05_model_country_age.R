...
df_event_1 <- df_sample_0 %>%
        select(country,pid,year,ln_hourly_wage,unmp,temp,perm,age,matches("event_1")) %>%
        rename(event_time_pos = event_1_time_pos)

df_event_2 <- df_sample_0 %>%
        select(country,pid,year,ln_hourly_wage,unmp,temp,perm,age,matches("event_2")) %>%
        rename(event_time_pos = event_2_time_pos)

df_event_3 <- df_sample_0 %>%
        select(country,pid,year,ln_hourly_wage,unmp,temp,perm,age,matches("event_3")) %>%
        rename(event_time_pos = event_3_time_pos)

df_event_4 <- df_sample_0 %>%
        select(country,pid,year,ln_hourly_wage,unmp,temp,perm,age,matches("event_4")) %>%
...
for(c in country) {
        ...
        print(screenreg(list(model_event_1_country,model_event_4_country,model_event_2_country,model_event_3_country),
        ...
        texreg(list(model_event_1_country,model_event_4_country,model_event_2_country,model_event_3_country),
                        custom.model.names = c("Event 1 (T-P)", "Event 4 (P-T)", "Event 2 (U-P)", "Event 3 (U-T)"),
                        custom.coef.names = c("Age", "Pre event (-2)", "Pre event (-1)", "Event", "Post event (+1)", "Post event (+2)", "Post event (+3)", "Post event (+4)", "Post event ($>=5$)"),
                        file = paste0(tables,"table_model_2_output_country.tex"),
                        table = FALSE,
                        include.rsquared = TRUE, include.adjrs = FALSE,include.nobs = FALSE,include.rmse=FALSE,include.groups=FALSE,
                        omit.coef = c("year|Intercept"))
}
...
ggsave(paste0(graphs,"graph_model_2_output_country.pdf"), height = 4, width = 8, plot = last_plot())
