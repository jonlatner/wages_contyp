...
df_country <- df_original %>%
        filter(country == "DE") %>%
        select(country,pid,year,age,ln_hourly_wage,unmp,temp,perm,event_t_p_step,event_p_t_step,event_t_p_yes_final,event_p_t_yes_final,event_t_p_time_pos,event_p_t_time_pos,unemployment_rate) %>%
        filter(unmp == 0)  %>%
        group_by(country, pid) %>%
        mutate(temp_2 = event_t_p_step + event_p_t_step,
               number = row_number(),
               max = max(number),
...
# FE model
# model_fe <- feis(ln_hourly_wage ~ age + unemployment_rate + temp_2 + factor(year) | 1,
...
# FEIS model
# model_feis <- feis(ln_hourly_wage ~ age + unemployment_rate + temp_2 + factor(year) | year,
#                    data = data.frame(df_country), 
#                    robust = TRUE,
#                    id = "pid")
# 
...
model_1 <- feis(ln_hourly_wage ~ age + unemployment_rate + event_t_p_time_pos + event_p_t_time_pos + as.factor(year) | 1,
                data = data.frame(df_country), 
                robust = TRUE,
                id = "pid")

model_p_t <- feis(ln_hourly_wage ~ age + unemployment_rate + event_p_t_time_pos + as.factor(year) | 1,
                data = data.frame(df_country), 
                robust = TRUE,
                id = "pid")
...
# df_output$event <- "FEIS + IF"
# df_output$country <- c
# df_yhat <- rbind(df_yhat,df_output)

library(texreg)

screenreg(list(model_1, model_p_t),
...
saveRDS(df_yhat, file = paste0(results, "df_yhat_first_contyp_single_country.rds"))
