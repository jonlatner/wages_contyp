...
df_event_u_p_01 <- df_original %>%
        ...
        filter((event_u_p_yes_final == 0 & unmp == 0) | event_u_p_yes_final == 1) %>%
...
df_event_u_p_01$event_time_pos <- relevel(factor(df_event_u_p_01$event_time_pos), ref = "0")
model_event_u_p_01 <- feis(ln_hourly_wage ~ age + unemployment_rate + event_time_pos + as.factor(year) | 1,
                        data = data.frame(df_event_u_p_01), 
                        robust = TRUE,
                        id = "pid")

# model - baseline is 1 period before event.  in annual data this is event_time_pos==2
df_event_u_p_02$event_time_pos <- relevel(factor(df_event_u_p_02$event_time_pos), ref = "0")
model_event_u_p_02 <- feis(ln_hourly_wage ~ age + unemployment_rate + event_time_pos + as.factor(year) | 1,
                        data = data.frame(df_event_u_p_02), 
                        robust = TRUE,
                        id = "pid")
...
screenreg(
        list(model_event_u_p_01, model_event_u_p_02),
        table = FALSE,
