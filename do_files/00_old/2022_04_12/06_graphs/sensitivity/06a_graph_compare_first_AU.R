...
# Contyp
df_contyp_1 <- readRDS(paste0(results,"df_yhat_first_contyp.rds"))
df_contyp_1 <- df_contyp_1 %>%
        filter(country=="AU") %>%
        mutate(type = "All temporary")

df_contyp_2 <- readRDS(paste0(results,"df_yhat_first_contyp_AU.rds"))
df_contyp_2 <- df_contyp_2 %>%
        filter(country=="AU") %>%
        mutate(type = "FTC only")

# Unemployment
df_unmp_1 <- readRDS(paste0(results,"df_yhat_first_unmp.rds"))
df_unmp_1 <- df_unmp_1 %>%
        filter(country=="AU") %>%
        mutate(type = "All temporary")

df_unmp_2 <- readRDS(paste0(results,"df_yhat_first_unmp_AU.rds"))
df_unmp_2 <- df_unmp_2 %>%
        filter(country=="AU") %>%
        mutate(type = "FTC only")
...
ggsave(paste0(graphs,"graph_sensitivity_AU.pdf"), height = 4, width = 8, plot = last_plot())
