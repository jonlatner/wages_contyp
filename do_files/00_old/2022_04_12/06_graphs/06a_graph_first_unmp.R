...
df_original <- readRDS(paste0(results,"df_yhat_first_unmp.rds"))
...
df_graph <- df_original %>%
        ...
        mutate(event = ifelse(term == "event_u_p_time_pos3", yes = "u_p",
                              ifelse(term == "event_u_t_time_pos3", yes = "u_t", no = event))) %>%
...
ggsave(paste0(graphs,"graph_first_unmp_event.pdf"), height = 4, width = 8, plot = last_plot())
...
ggsave(plot=p4,paste0(graphs,"graph_first_unmp_event_better.pdf"), height = 4, width = 8)
...
ggsave(paste0(graphs,"graph_first_unmp_post_free_scale.pdf"), height = 4, width = 8, plot = last_plot())
...
ggsave(plot=p4,paste0(graphs,"graph_first_unmp_post_better.pdf"), height = 4, width = 8)
