...
for (e in event) {
        for(c in country_all) {
                load(paste0(results,"model_event_log_",e,"_country_",c,".Rdata"))
...
for (e in event) {
        ...
        #        custom.coef.names = c("Unmp rate", "Age","Pre event (-2)", "Event", "Post event (+1)", "Post event (+2)", "Post event (+3)", "Post event (+4)"),
        ...
        texreg(list(model_AU,model_CH,model_DE,model_JP,model_KO,`model_NE-LISS`,model_UK,`model_NE-LSP`,model_IT),
               custom.model.names = c("AU", "CH", "DE", "JP", "KO", "UK", "NE-LSP","IT"),
               custom.coef.names = c("Unmp rate", "Age","Pre event (-2)", "Event", "Post event (+1)", "Post event (+2)", "Post event (+3)", "Post event (+4)"),
               file = paste0(tables,"table_output_log_event_",e,".tex"),
...
for(c in country_ann) {
        ...
        texreg(list(model_event_t_p_country,model_event_p_t_country,model_event_u_p_country,model_event_u_t_country),
               custom.model.names = c("Temp to Perm", "Perm to Temp", "Unmp to Perm", "Unmp to Temp"),
               custom.coef.names = c("Unmp rate", "Age","Pre event (-2)", "Event", "Post event (+1)", "Post event (+2)", "Post event (+3)", "Post event (+4)"),
               file = paste0(tables,"table_output_log_country_",c,".tex"),
               table = FALSE,include.ci = FALSE,digits=3,
               custom.note = ("\\parbox{1.1\\linewidth}{\\vspace{2pt}%stars. \\\\
                              Note: Reference period is 1 period before event.  SE are cluster robust.  Year fixed effects not shown.}"),
               include.rsquared = TRUE, include.adjrs = FALSE,include.nobs = TRUE,include.rmse=FALSE,include.groups=TRUE,
               omit.coef = c("year|Intercept"))
}
...
for(c in country_bi) {
        ...
        texreg(list(model_event_t_p_country,model_event_p_t_country,model_event_u_p_country,model_event_u_t_country),
               custom.model.names = c("Temp to Perm", "Perm to Temp", "Unmp to Perm", "Unmp to Temp"),
               custom.coef.names = c("Unmp rate", "Age", "Event", "Post event (+2)", "Post event (+4)"),
               file = paste0(tables,"table_output_log_country_",c,".tex"),
               table = FALSE,include.ci = FALSE,digits=3,
               custom.note = ("\\parbox{1.1\\linewidth}{\\vspace{2pt}%stars. \\\\
                              Note: Reference period is 1 period before event.  SE are cluster robust.  Year fixed effects not shown.}"),
