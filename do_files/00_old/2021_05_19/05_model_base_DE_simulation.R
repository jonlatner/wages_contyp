...
# df_sample_small <- df_sample_2 %>%
#         filter(id < 100)
# View(df_sample_small)

ggplot(data = df_sample_small, aes(x = year, y = ln_hourly_wage, group = pid, color = as.factor(pid), shape = as.factor(temp))) +
        geom_line(size = 1) +
        geom_point(size = 2) +
        theme_bw()

df_sample_small <- data.frame(df_sample_small)

model_0 <- feis(ln_hourly_wage ~ temp + factor(year) | 1, 
                   data = df_sample_small, 
                   id = "pid")

model_1 <- feis(ln_hourly_wage ~ temp + factor(post_temp) | 1, 
                data = df_sample_small, 
                id = "pid")

model_2 <- feis(ln_hourly_wage ~ temp + factor(post_temp) | year, 
                data = df_sample_small, 
                id = "pid")
...
#                 colnames(output_table) <- "estimate"
#                 output_table$std.error <- estimate(summary(feis_model))[, "Std. Error"]
