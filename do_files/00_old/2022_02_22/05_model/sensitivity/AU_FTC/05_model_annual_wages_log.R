...
df_original <- readRDS(paste0(data_files,"au_sample_v02.rds"))
...
for(c in country_ann) {
        ...
        for (e in event) {
                ...
                # assign(paste0("model_annual_wages_log_event_",e,"_country_",c),model)
                save(model, file=paste0(results,"model_annual_wages_log_event_",e,"_country_",c,".Rdata"))
...
saveRDS(df_yhat, file = paste0(results, "df_yhat_annual_wages_log.rds"))
