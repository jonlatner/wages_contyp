...
for(c in country_bi) {
        ...
        for (e in event) {
                ...
                model <- feis(ln_hourly_wage ~ event_time_pos + as.factor(year) | 1,
                                   data = df_country, 
                                   robust = TRUE,
                                   id = "pid")
                
                assign(paste0("model_event_",e,"_country_",c),model)
                save(model, file=paste0(results,"model_1_first_event_",e,"_country_",c,".Rdata"))
...
for (e in event) {
        ...
        # screenreg(list(model_AU,model_CH,model_DE,model_JP,model_KO,`model_NE-LISS`,model_UK,`model_NE-LSP`,model_IT),
