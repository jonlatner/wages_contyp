...
for(c in country_ann) {
        ...
        # Temp to perm
        model <- feis(ln_hourly_wage ~ age + unemployment_rate + event_t_p_time_pos + as.factor(year) | 1,
        ...
        # Perm to temp
        model <- feis(ln_hourly_wage ~ age + unemployment_rate + event_p_t_time_pos + as.factor(year) | 1,
                      data = data.frame(df_country), 
                      robust = TRUE,
                      id = "pid")
...
saveRDS(df_yhat, file = paste0(results, "df_yhat_first_contyp_original.rds"))
