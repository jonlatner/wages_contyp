...
df_event_t_p <- df_original %>%
        select(country,pid,year,age,ln_hourly_wage,unmp,temp,perm,matches("event_t_p"),unemployment_rate,male) %>%
        filter(unmp == 0) %>%
...
df_event_p_t <- df_original %>%
        select(country,pid,year,age,ln_hourly_wage,unmp,temp,perm,matches("event_p_t"),unemployment_rate,male) %>%
        filter(unmp == 0) %>%
...
for(c in country_ann) {
        ...
        for (e in event) {
                ...
                model_m <- feis(ln_hourly_wage ~ age + unemployment_rate + event_time_pos + as.factor(year) | 1,
