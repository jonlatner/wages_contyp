...
table(df_sample_02$event_t_p_time_pos)

df_sample_02 %>% group_by(event_t_p_time_pos) %>% summarise(mean = mean(ln_hourly_wage))
...
table(df_sample_08$event_p_t_time_pos)

df_sample_08 %>% group_by(event_p_t_time_pos) %>% summarise(mean = mean(ln_hourly_wage))
