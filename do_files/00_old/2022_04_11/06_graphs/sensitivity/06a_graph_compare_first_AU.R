...
df_contyp_2 <- readRDS(paste0(results,"df_yhat_first_hourly_wages_log_contyp_AU.rds"))
df_contyp_2 <- df_contyp_2 %>%
        filter(country=="AU") %>%
        mutate(type = "FTC + casual")
...
df_unmp_2 <- readRDS(paste0(results,"df_yhat_first_hourly_wages_log_unmp_AU.rds"))
df_unmp_2 <- df_unmp_2 %>%
        filter(country=="AU") %>%
        mutate(type = "FTC + casual")
