...
# Contyp
df_contyp_feis <- readRDS(paste0(results,"df_yhat_first_hourly_wages_log_contyp_feis.rds"))
df_contyp_feis <- df_contyp_feis %>%
        filter(country!="NE-LISS") %>%
        mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country),
               type = "FEIS")

df_contyp_fe <- readRDS(paste0(results,"df_yhat_first_hourly_wages_log_contyp.rds"))
...
# Unemployment
df_unmp_feis <- readRDS(paste0(results,"df_yhat_first_hourly_wages_log_unmp_feis.rds"))
df_unmp_feis <- df_unmp_feis %>%
        filter(country!="NE-LISS") %>%
        mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country),
               type = "FEIS")

df_unmp_fe <- readRDS(paste0(results,"df_yhat_first_hourly_wages_log_unmp.rds"))
...
arrange(df_graph,country,event,post,type)
...
ggsave(paste0(graphs,"graph_contyp_post_compare_first_feis.pdf"), height = 4, width = 8, plot = last_plot())
...
ggsave(plot=p4,paste0(graphs,"graph_unmp_post_compare_first_feis.pdf"), height = 4, width = 8)
