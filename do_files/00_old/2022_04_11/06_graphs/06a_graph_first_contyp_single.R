...
df_original <- readRDS(paste0(results,"df_yhat_first_contyp_single_country.rds"))
...
df_graph <- df_original %>%
        filter(term == "temp_2" | term == "event_t_p_time_pos3" | term == "event_p_t_time_pos3") %>%
        mutate(event = ifelse(term == "event_t_p_time_pos3", yes = "t_p",
                              ifelse(term == "event_p_t_time_pos3", yes = "p_t", no = event))) %>%
...
# ggsave(paste0(graphs,"graph_output_hourly_log_contyp_event.pdf"), height = 4, width = 8, plot = last_plot())
...
# ggsave(paste0(graphs,"graph_output_hourly_log_contyp_post.pdf"), height = 4, width = 8, plot = last_plot())
...
# ggsave(paste0(graphs,"graph_output_hourly_log_contyp_post_free_scale.pdf"), height = 4, width = 8, plot = last_plot())
