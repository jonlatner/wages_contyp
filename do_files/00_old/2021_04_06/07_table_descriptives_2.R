...
df_sample_2 <- df_sample_1 %>%
  select(country,ln_hourly_wage)

df_table_3 <- df_sample_2 %>%
  group_by(country) %>%
  summarise_all(c(mean,sd)) %>%
  ungroup() %>%
  mutate(fn2=sprintf("(%.3f)",fn2))
...
df_total <- format(df_total, big.mark=",", scientific=FALSE)
df_total
...
rownames(df_table) <- c("\\\\[-1.8ex] 
                        \\multicolumn{10}{l}{\\emph{Dependent variable - Hourly wage (LN)}} \\\\
                        \\hspace{10mm}Average",
                        "\\hspace{10mm}(SD)",
                        "\\\\[-1.8ex] 
                        ...
                        \\hspace{10mm}Male",
                        "\\\\[-1.8ex] 
                        \\multicolumn{10}{l}{\\emph{Years after temp contract $|$ temp contract}} \\\\
                        \\hspace{10mm}1 Year",
                        "\\hspace{10mm}2 Years",
                        "\\hspace{10mm}3 Years",
                        "\\hspace{10mm}4 Years",
...
digits_1 <- matrix(c(2),nrow=1,ncol=10)
digits_1 <- matrix(c(2),nrow=18,ncol=10)
