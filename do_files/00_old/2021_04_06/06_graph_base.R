...
df_model_output_base_unbalanced <- readRDS(paste0(results, "unbalanced/results_a_predicted.rds")) %>%
        mutate(model = "model 1 (unbalanced)")
df_model_output_full <- readRDS(paste0(results, "results_b_predicted.rds")) %>%
        select(-residual.scale,-df) %>%
        mutate(model = "model 2")

df_model_output <- rbind(df_model_output_base,df_model_output_base_unbalanced,df_model_output_full)
...
ggplot(data = df_graph, aes(x = study_period, y = fit, color = model)) +
        ...
        geom_line(size = 1) +
        geom_point(size = 2) +
        scale_color_manual(values=c("gray50", "gray80", "black")) +
...
ggsave(paste0(graphs,"graph_one_unit.pdf"), height = 5, width = 7, plot = last_plot())
