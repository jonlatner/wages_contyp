...
filter(df_sample_1,pid==100037) %>% select(country,study_period,pid,year,temp,matches("temp"))
...
filter(df_sample_2,pid==19272) %>% select(country,study_period,pid,year,temp,matches("temp"))
filter(df_sample_2,pid==100037) %>% select(country,study_period,pid,year,temp,matches("temp"))
filter(df_sample_2,pid==6273142) %>% select(country,study_period,pid,year,temp,matches("temp"))


t <- with(subset(df_sample_2, post_temp>0), prop.table(table(post_temp)))
t <- data.frame(t)
...
saveRDS(df_sample_2, file = paste0(data_files, "df_sample_clean_unbalanced.rds"))
