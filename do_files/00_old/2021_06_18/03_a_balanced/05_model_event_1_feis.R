...
for(c in country) {
        ...
        for(y in year) {
                ...
                model <- feis(ln_hourly_wage ~ unmp + temp + event_1a | year, 
                              data = df_period, 
                              id = "pid")
...
for(c in country) {
        ...
        for(y in year) {
                ...
                # model
                df_period <- data.frame(df_period)
                model <- feis(ln_hourly_wage ~ temp + event_1a | year, 
                              data = df_period, 
                              id = "pid")
...
df_model_yhat$model <- "event1_feis_if"
df_model_yhat$post <- 0
df_model_yhat$event <- 1

# Save data sets ----

write.csv(df_output, file = paste0(results, "output_event1_feis_if.csv"))
write.csv(df_model_yhat, file = paste0(results, "yhat_event1_feis_if.csv"))
