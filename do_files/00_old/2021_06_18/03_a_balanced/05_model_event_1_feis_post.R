...
df_sample_1 <- df_sample_0 %>%
        select(country,study_period,pid,year,period,ln_hourly_wage,unmp,temp,event_1a,post_event_1)
...
for(c in country) {
        ...
        for(y in year) {
                ...
                output$model <- "event1_feis_if_post"
...
df_model_yhat$model <- "event1_feis_if_post"
df_model_yhat$event <- 1

write.csv(df_output, file = paste0(results, "output_event1_feis_if_post.csv"))
write.csv(df_model_yhat, file = paste0(results, "yhat_event1_feis_if_post.csv"))
