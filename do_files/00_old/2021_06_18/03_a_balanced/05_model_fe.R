...
df_sample_1 <- df_sample_0 %>%
        select(country,study_period,pid,year,period,ln_hourly_wage,unmp,temp)
...
# netherlands has no unemployed
# with(subset(df_sample_1,country=="NE-LISS"), table(unmp,study_period))
country <- c("NE-LISS")
for(c in country) {
        print(c)
        ...
        for(y in year) {
                ...
                model <- feis(ln_hourly_wage ~ temp + period_2 + period_3 + period_4 + period_5 + period_6 + period_7 | 1, 
                                   data = df_period, 
                                   id = "pid")
...
for(c in country) {
        ...
        for(y in year) {
                ...
                # model
                df_period <- data.frame(df_period)
                model <- feis(ln_hourly_wage ~ unmp + temp + period_2 + period_3 + period_4 | 1, 
                              data = df_period, 
                              id = "pid")
...
for(c in country) {
        ...
        for(y in year) {
                ...
                # model
                df_period <- data.frame(df_period)
                model <- feis(ln_hourly_wage ~ unmp + temp + period_2 + period_3 + period_4 + period_5 + period_6 + period_7 | 1, 
                              data = df_period, 
                              id = "pid")
...
for(c in country) {
        ...
        for(y in year) {
                ...
                # model
                df_period <- data.frame(df_period)
                model <- feis(ln_hourly_wage ~ unmp + temp + period_2 + period_3 + period_4 + period_5 + period_6 | 1, 
                              data = df_period, 
                              id = "pid")
...
df_model_yhat$model <- "fe"
df_model_yhat$post <- 0
df_model_yhat$event <- 0

# Save data sets ----

write.csv(df_output, file = paste0(results, "output_fe.csv"))
write.csv(df_model_yhat, file = paste0(results, "yhat_fe.csv"))
