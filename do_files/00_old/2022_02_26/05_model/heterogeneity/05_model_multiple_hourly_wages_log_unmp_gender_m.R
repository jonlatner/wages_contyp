...
df_event_u_p <- df_original %>%
        select(country,pidseq,year,age,ln_hourly_wage,unmp,temp,perm,matches("event_u_p"),unemployment_rate) %>%
        filter((event_u_p_yes == 0 & unmp == 0) | (event_u_p_yes == 1 & event_u_p_drop == 0)) %>%
...
df_event_u_t <- df_original %>%
        select(country,pidseq,year,age,ln_hourly_wage,unmp,temp,perm,matches("event_u_t"),unemployment_rate) %>%
...
saveRDS(df_yhat, file = paste0(results, "df_yhat_multiple_hourly_wages_log_unmp_gender_m.rds"))
