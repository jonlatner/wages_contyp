...
df_sample_2 <- df_sample_1 %>%
        ...
        mutate(event_t_p = ifelse(perm == 1 & lag(temp,1) == 1 & row_number()>1 & year == lag(year,1,NA)+year_lag, yes = 1, no = NA),
               ...
               unmp_ever = ifelse(unmp == 1, yes = 1, no = NA),
               unmp_ever = na.locf(unmp_ever,na.rm = FALSE),
               unmp_ever = ifelse(last(unmp_ever == 1), yes = 1, no = 0),
        ) %>%
        ungroup() 

View(filter(df_sample_2,unmp_ever == 1))

table(df_sample_2$unmp, useNA = "ifany")
table(df_sample_2$event_u_t, useNA = "ifany")
table(df_sample_2$event_u_p, useNA = "ifany")
