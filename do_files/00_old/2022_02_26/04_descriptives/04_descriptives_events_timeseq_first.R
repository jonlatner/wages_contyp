...
df_event_t_p <- df_sample_0 %>%
        select(country, pid, year, unmp,temp, perm, matches("event_t_p")) %>%
        filter(event_t_p_yes==1 & event_t_p_drop == 0) %>%
        mutate(event_t_p_time = ifelse(event_t_p_yes == 1, yes = year - event_t_p_year, no = NA),
               event_t_p_time = ifelse(event_t_p_time < -2, yes = NA,
                                       ifelse(event_t_p_time > 4, yes = NA, no = event_t_p_time)),
               ) %>%
        filter(!is.na(event_t_p_time))

df_event_p_t <- df_sample_0 %>%
        select(country, pid, year, unmp,temp, perm, matches("event_p_t")) %>%
        filter(event_p_t_yes==1 & event_p_t_drop == 0) %>%
        mutate(event_p_t_time = ifelse(event_p_t_yes == 1, yes = year - event_p_t_year, no = NA),
               event_p_t_time = ifelse(event_p_t_time < -2, yes = NA,
                                       ifelse(event_p_t_time > 4, yes = NA, no = event_p_t_time)),
        ) %>%
        filter(!is.na(event_p_t_time))


df_event_u_t <- df_sample_0 %>%
        select(country, pid, year, unmp,temp, perm, matches("event_u_t")) %>%
        filter(event_u_t_yes==1 & event_u_t_drop == 0) %>%
        mutate(event_u_t_time = ifelse(event_u_t_yes == 1, yes = year - event_u_t_year, no = NA),
               event_u_t_time = ifelse(event_u_t_time < -2, yes = NA,
                                       ifelse(event_u_t_time > 4, yes = NA, no = event_u_t_time)),
        ) %>%
        filter(!is.na(event_u_t_time))


df_event_u_p <- df_sample_0 %>%
        select(country, pid, year, unmp,temp, perm, matches("event_u_p")) %>%
        filter(event_u_p_yes==1 & event_u_p_drop == 0) %>%
        mutate(event_u_p_time = ifelse(event_u_p_yes == 1, yes = year - event_u_p_year, no = NA),
               event_u_p_time = ifelse(event_u_p_time < -2, yes = NA,
                                       ifelse(event_u_p_time > 4, yes = NA, no = event_u_p_time)),
        ) %>%
        filter(!is.na(event_u_p_time))
