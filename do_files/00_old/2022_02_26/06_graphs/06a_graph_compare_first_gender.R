...
# Contyp
df_contyp_f <- readRDS(paste0(results,"df_yhat_multiple_hourly_wages_log_contyp_gender_f.rds"))
df_contyp_f <- df_contyp_f %>%
        filter(country!="NE-LISS") %>%
        mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country),
               type = "Female")

df_contyp_m <- readRDS(paste0(results,"df_yhat_multiple_hourly_wages_log_contyp_gender_m.rds"))
...
# Unemployment
df_unmp_f <- readRDS(paste0(results,"df_yhat_multiple_hourly_wages_log_unmp_gender_f.rds"))
df_unmp_f <- df_unmp_f %>%
        filter(country!="NE-LISS") %>%
        mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country),
               type = "Female")

df_unmp_m <- readRDS(paste0(results,"df_yhat_multiple_hourly_wages_log_unmp_gender_m.rds"))
df_unmp_m <- df_unmp_m %>%
        filter(country!="NE-LISS") %>%
        mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country),
               type = "Male")
...
ggsave(paste0(graphs,"graph_unmp_post_compare_first_gender.pdf"), height = 4, width = 8, plot = last_plot())
