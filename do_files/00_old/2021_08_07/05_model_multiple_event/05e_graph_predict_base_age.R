...
df_yhat <- readRDS(paste0(data_files,"df_yhat_multiple_base_age.rds"))

df_yhat <- df_yhat %>%
        filter(str_detect(term, 'event_time')) %>%
        rename(fit=estimate,
               se.fit=std.error)
df_yhat$post <- gsub("^.*_", "", df_yhat$term)
...
ggsave(paste0(graphs,"graph_model_2_multiple_output_country_DE.pdf"), height = 4, width = 8, plot = last_plot())
...
ggsave(paste0(graphs,"graph_model_2_multiple_output_event_temp.pdf"), height = 4, width = 8, plot = last_plot())
...
ggsave(paste0(graphs,"graph_model_2_multiple_output_event_unmp.pdf"), height = 4, width = 8, plot = last_plot())
...
ggsave(paste0(graphs,"graph_model_2_multiple_output_event_unmp_post_event.pdf"), height = 4, width = 8, plot = last_plot())
