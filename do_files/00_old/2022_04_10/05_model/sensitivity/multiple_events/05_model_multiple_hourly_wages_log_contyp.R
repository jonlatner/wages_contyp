...
df_event_t_p <- df_original %>%
        select(country,pid,pidseq,year,age,ln_hourly_wage,unmp,temp,perm,matches("event_t_p"),unemployment_rate) %>%
        filter(unmp == 0)  %>%
...
df_event_p_t <- df_original %>%
        select(country,pid,pidseq,year,age,ln_hourly_wage,unmp,temp,perm,matches("event_p_t"),unemployment_rate) %>%
        filter(unmp == 0)  %>%
...
saveRDS(df_yhat, file = paste0(results, "df_yhat_multiple_hourly_wages_log_contyp.rds"))
