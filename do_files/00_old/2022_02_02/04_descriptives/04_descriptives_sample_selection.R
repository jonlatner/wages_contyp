...
df_filter_3a <- readRDS(file = paste0(data_files,"3a_df_filter_steps.rds"))
df_filter_3b <- readRDS(file = paste0(data_files,"3b_df_filter_steps.rds"))

df_filter <- rbind(df_filter_3a,df_filter_3b)
rm(df_filter_3a,df_filter_3b)
                                                    ...
                                                    ifelse(step == 5, yes = "Unemployed or employed with monthly hours $>= 1$",
                                                           ifelse(step == 6, yes = "Non missing education or gender",
                                                                  ifelse(step == 7, yes = "Drop observations with hourly wages in the top/bottom 0.005 percentile",
                                                                         ifelse(step == 8, yes = "Must be observable for at least 2 consecutive years",
                                                                                ifelse(step == 9, yes = "Must be observable for at least 3 consecutive years",
                                                                                       no = NA)))))))))))
df_filter
