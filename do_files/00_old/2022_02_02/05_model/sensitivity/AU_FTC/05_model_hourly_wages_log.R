...
df_event_t_p <- df_original %>%
        select(country,pid,pidseq,year,age,hourly_wage,ln_hourly_wage,unmp,temp,perm,matches("event_t_p"),unemployment_rate) %>%
        rename(event_time_pos = event_t_p_time_pos) %>%
        filter(!is.na(event_time_pos))

df_event_u_p <- df_original %>%
        select(country,pid,pidseq,year,age,hourly_wage,ln_hourly_wage,unmp,temp,perm,matches("event_u_p"),unemployment_rate) %>%
        rename(event_time_pos = event_u_p_time_pos) %>%
        filter(!is.na(event_time_pos))

df_event_u_t <- df_original %>%
        select(country,pid,pidseq,year,age,hourly_wage,ln_hourly_wage,unmp,temp,perm,matches("event_u_t"),unemployment_rate) %>%
        rename(event_time_pos = event_u_t_time_pos) %>%
        filter(!is.na(event_time_pos))

df_event_p_t <- df_original %>%
        select(country,pid,pidseq,year,age,hourly_wage,ln_hourly_wage,unmp,temp,perm,matches("event_p_t"),unemployment_rate) %>%
...
for(c in country_ann) {
        ...
        for (e in event) {
                ...
                # assign(paste0("model_hourly_wages_log_event_",e,"_country_",c),model)
...
saveRDS(df_yhat, file = paste0(results, "df_yhat_hourly_wages_log.rds"))
