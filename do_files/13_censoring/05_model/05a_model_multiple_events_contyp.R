...
df_original <- df_original %>%
        select(country,pid,pidseq,year,age,ln_hourly_wage,unmp,temp,perm,event_t_p_yes_final,event_p_t_yes_final,event_t_p_time_pos,event_p_t_time_pos,event_t_p_time,event_p_t_time,unemployment_rate) %>%
        filter(unmp == 0)  %>%
...
for(c in country_ann) {
        ...
        df_country <- df_original %>%
                filter(country == c) %>%
          mutate(drop = ifelse(
            (event_t_p_time< -1 & event_t_p_yes_final == 1) | 
              (event_t_p_time> 4 & event_t_p_yes_final == 1) |
              (event_p_t_time< -1 & event_p_t_yes_final == 1) |
              (event_p_t_time> 4 & event_p_t_yes_final == 1)
            , 1, 0)) %>%
          filter(drop == 0)
...
for(c in country_bi) {
  ...
  df_country <- df_original %>%
    filter(country == c) %>%
    mutate(drop = ifelse(
      (event_t_p_time< -2 & event_t_p_yes_final == 1) | 
        (event_t_p_time> 4 & event_t_p_yes_final == 1) |
        (event_p_t_time< -2 & event_p_t_yes_final == 1) |
        (event_p_t_time> 4 & event_p_t_yes_final == 1)
      , 1, 0)) %>%
    filter(drop == 0)
...
saveRDS(df_yhat, file = paste0(results, "df_yhat_multiple_events_contyp.rds"))
