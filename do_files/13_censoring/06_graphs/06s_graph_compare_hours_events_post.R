...
# Main sample (without censoring)
df_multiple_contyp <- readRDS(paste0(results,"censoring/df_yhat_multiple_events_contyp.rds"))
df_multiple_contyp <- df_multiple_contyp %>%
  filter(country!="NE-LISS") %>%
  mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country))

df_multiple_unmp <- readRDS(paste0(results,"censoring/df_yhat_multiple_events_unmp.rds"))
df_multiple_unmp <- df_multiple_unmp %>%
  filter(country!="NE-LISS") %>%
  mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country))

df_censoring_1 <- rbind(df_multiple_contyp,df_multiple_unmp) %>%
  filter(model == "FE + DIF")
df_censoring_1$type = "Sensitivity sample (with censoring)"
...
df_censoring_2 <- rbind(df_multiple_contyp,df_multiple_unmp) %>%
        filter(model == "FE + DIF")
df_censoring_2$type = "Main sample (without censoring)"

df_append <- rbind(df_censoring_1,df_censoring_2)
...
ggsave(paste0(graphs,"graph_sensitivity_censoring_post.pdf"), height = 6, width = 9, plot = last_plot())
