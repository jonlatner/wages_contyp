...
df_censoring_1 <- readRDS(paste0(results,"df_yhat_multiple_events_unmp.rds"))
df_censoring_1 <- df_censoring_1 %>%
        filter(country!="NE-LISS") %>%
        mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country))
df_censoring_1$type = "Main sample"

df_censoring_2 <- readRDS(paste0(results,"censoring/df_yhat_multiple_events_unmp.rds"))
df_censoring_2 <- df_censoring_2 %>%
  filter(country!="NE-LISS") %>%
  mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country))
df_censoring_2$type = "Main sample (with censoring)"
...
p <- ggplot(data=df_graph, aes(x=event, y=estimate, color = type)) +
        facet_wrap(~country_name,drop=TRUE, nrow = 2, scales = "free") +
...
p
ggsave(plot=p,paste0(graphs,"graph_sensitivity_compare_unmp_censoring.pdf"), height = 6, width = 9)
