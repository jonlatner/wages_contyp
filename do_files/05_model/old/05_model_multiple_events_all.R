...
for(c in country_ann) {
        ...
        # FE model
        model_fe <- feis(ln_hourly_wage ~ age + unemployment_rate + unmp + temp + factor(year) | 1,
        ...
        # FEIS model
        model_feis <- feis(ln_hourly_wage ~ age + unemployment_rate + unmp + temp + factor(year) | year,
                           data = data.frame(df_country), 
                           robust = TRUE,
                           id = "pidseq")
        ...
        # Unmp to perm
        model <- feis(ln_hourly_wage ~ age + unemployment_rate + event_u_p_time_pos + event_u_t_time_pos + event_t_p_time_pos + event_p_t_time_pos + as.factor(year) | 1,
                      data = data.frame(df_country), 
                      robust = TRUE,
                      id = "pidseq")
        
        df_output <- tidy_parameters(model)
        df_output$model <- "FEIF"
        df_output$country <- c
        df_yhat <- rbind(df_yhat,df_output)
}
...
saveRDS(df_yhat, file = paste0(results, "df_yhat_multiple_events.rds"))
