...
df_original <- df_original %>%
        ...
        mutate(age_cat = max(age)) %>%
        filter(age_cat > 34 & age < 45) %>%
...
for(c in country_ann) {
        ...
        # FEIS model
        model_feis <- feis(ln_hourly_wage ~ age + unemployment_rate + temp | year,
                           data = data.frame(df_country), 
                           robust = TRUE,
                           id = "pidseq")
...
saveRDS(df_yhat, file = paste0(results, "df_yhat_multiple_events_contyp_age_cat_M.rds"))
