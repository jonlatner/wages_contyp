...
for(c in country_ann) {
        ...
        model <- feis(ln_hourly_wage ~ age + unemployment_rate + event_t_p_time_pos + event_p_t_time_pos + as.factor(year) | year,
...
saveRDS(df_yhat, file = paste0(results, "df_yhat_multiple_events_contyp_feis.rds"))
