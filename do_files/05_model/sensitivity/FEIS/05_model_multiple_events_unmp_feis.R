...
for(c in country_ann) {
        ...
        # Unmp to perm
        model <- feis(ln_hourly_wage ~ age + unemployment_rate + event_u_p_time_pos + event_u_t_time_pos + as.factor(year) | year,
...
saveRDS(df_yhat, file = paste0(results, "df_yhat_multiple_events_unmp_feis.rds"))
