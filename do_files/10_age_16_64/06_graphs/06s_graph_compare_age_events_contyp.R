...
df_age_2 <- readRDS(paste0(results,"age_16_64/df_yhat_multiple_events_contyp.rds"))
df_age_2 <- df_age_2 %>%
  filter(country!="NE-LISS") %>%
  mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country))
...
ggsave(plot=p,paste0(graphs,"graph_sensitivity_compare_contyp_age.pdf"), height = 6, width = 9)
