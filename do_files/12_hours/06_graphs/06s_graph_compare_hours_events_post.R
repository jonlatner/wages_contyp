...
# Hours (20 - 320)
df_multiple_contyp <- readRDS(paste0(results,"hours/df_yhat_multiple_events_contyp.rds"))
df_multiple_contyp <- df_multiple_contyp %>%
  filter(country!="NE-LISS") %>%
  mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country))

df_multiple_unmp <- readRDS(paste0(results,"hours/df_yhat_multiple_events_unmp.rds"))
df_multiple_unmp <- df_multiple_unmp %>%
  filter(country!="NE-LISS") %>%
  mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country))

df_hours_1 <- rbind(df_multiple_contyp,df_multiple_unmp) %>%
  filter(model == "FE + DIF")
df_hours_1$type = "Sensitivity sample - monthly hours (20 - 320)"
...
df_hours_2 <- rbind(df_multiple_contyp,df_multiple_unmp) %>%
        filter(model == "FE + DIF")
df_hours_2$type = "Main sample - monthly hours (40 - 320)"

df_append <- rbind(df_hours_1,df_hours_2)
...
ggsave(paste0(graphs,"graph_sensitivity_hours_post.pdf"), height = 6, width = 9, plot = last_plot())
