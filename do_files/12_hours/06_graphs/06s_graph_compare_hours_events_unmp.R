...
df_hours_1 <- readRDS(paste0(results,"df_yhat_multiple_events_unmp.rds"))
df_hours_1 <- df_hours_1 %>%
        filter(country!="NE-LISS") %>%
        mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country))
df_hours_1$type = "Hours (40-320)"

df_hours_2 <- readRDS(paste0(results,"hours/df_yhat_multiple_events_unmp.rds"))
df_hours_2 <- df_hours_2 %>%
  filter(country!="NE-LISS") %>%
  mutate(country = ifelse(country == "NE-LSP", yes = "NE", no = country))
df_hours_2$type = "Hours (20-320)"
...
p
ggsave(plot=p,paste0(graphs,"graph_sensitivity_compare_unmp_hours.pdf"), height = 6, width = 9)
